...
namespace Com.Cos.Application
{
    ...
    /// 合作回复评论表业务
    /// </summary>
    public class CooperationReplyService:BaseService<CooperationReply>,ICooperationReplyService
    {
        ...
        /// <returns></returns>
        public async Task<(PageDto pageDto, List<CooperationReplyDto> dtoList)> FindListAsync(int exId, int userId, int pageIndex, int pageSize)
        {
            ...
            var v = from er in this.DbContext.CooperationReplies
                join m in this.DbContext.Members on er.UserId equals m.Id into mi
                from m in mi.DefaultIfEmpty()
                join er2 in this.DbContext.CooperationReplies on er.ParentId equals er2.Id into er2i
                ...
                select new CooperationReplyDto()
                {
                    Id = er.Id,
                    CoId = er.CoId,
                    UserId = er.UserId,
                    ...
                    Nickname = m.Nickname,
                    Text = er.Text,
                    AddTime = SysHelper.GetDateInterval(er.AddTime.ToLongTimeString()),
                    LikeNum = er.LikeNum,
        ...
        /// <returns></returns>
        public async Task<bool> AddAsync(CooperationReplyDto dto)
        {
            var model = new CooperationReply()
            {
                CoId = dto.CoId,
                UserId = dto.UserId,
                Text = dto.Text,
            ...
            model = await this.CooperationReplyRepository.AddAsync(model);
            return model.Id > 0;
        }
        /// <summary>
        /// 点赞
        ...
        {
            var model = await this.CooperationReplyRepository.FindAsync(er => er.Id == id);
            if (!model.LikeUser.Split(',', StringSplitOptions.RemoveEmptyEntries).Contains(userId.ToString()))  //没有点赞，则点赞
            {
                model.LikeNum++;
                model.LikeUser += userId + ",";
            }
            return await this.CooperationReplyRepository.UpdateAsync(model);
        }
    }
}