...
namespace Com.Cos.Web.Service.App_Public
{
    ...
    public class RecoverPassword : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            //发送注册邮件
            bool b = MailHelper.sendMail("您正在申请重设密码", mailBody.ToString(), EmailConfig.Instance._EmailName, new List<string>() { dt.Rows[0]["Email"].ToString() }, EmailConfig.Instance._EmailAgreement, EmailConfig.Instance._EmailUserName, EmailConfig.Instance._EmailPassword);
            if (b)
            {
